...
public class TestClusteringRook2 extends MatrixAnalysis {

	private Collection<Integer> interest;
	
	public TestClusteringRook2(Matrix m, Collection<Integer> interest){
		super(m);	
	...
	protected void doRun() {
		...
		//double[] sames = new double[99999999];
	...
	double getSame(Map<Double, Double> sames/*double[] sames,*/, double v){
		//System.out.println(sames.size());
		double nv = sames.get(v);
		//double nv = sames[(int) v];
		if(nv == -1){
			return v;
		}else{
