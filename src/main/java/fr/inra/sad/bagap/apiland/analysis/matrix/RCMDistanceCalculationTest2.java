...
public class RCMDistanceCalculationTest2 extends MatrixCalculation {
	...
	public RCMDistanceCalculationTest2(Matrix m, Friction f, Collection<Integer> values) {
		this(m, f, values, Raster.getNoDataValue());
	}

	public RCMDistanceCalculationTest2(Matrix m, Friction f, Collection<Integer> values, double threshold) {
		super(m);
		this.frictionMap = f;
		this.map = true;
		this.values = values;
	...
	public RCMDistanceCalculationTest2(Matrix m, Matrix f, Collection<Integer> values) {
		this(m, f, values, Raster.getNoDataValue());
	}

	public RCMDistanceCalculationTest2(Matrix m, Matrix f, Collection<Integer> values, double threshold) {
		super(m);
		this.frictionMat = f;
		this.map = false;
		this.values = values;
	...
	public void doRun() {
		...
		while (waits.size() > 0) {
			...
			treat = diffusion(treat, matrix(), dist, waits);
	...
	private int diffusion(int treat, Matrix m, Matrix dist, Map<Double, Set<Pixel>> waits) {

		Iterator<Entry<Double, Set<Pixel>>> iteEntry = waits.entrySet().iterator();
		Entry<Double, Set<Pixel>> entry = iteEntry.next();
		double dd = entry.getKey(); // récupération de la valeur de diffusion 
		
		Iterator<Pixel> itePixel = entry.getValue().iterator();
		Pixel p = itePixel.next(); // récupération du prochain point de diffusion
		itePixel.remove(); // suppression du pixel de la liste
		
		if(entry.getValue().size() == 0){ // si il n'y a plus de pixel à cette distance non traités, on supprime la liste associée
			iteEntry.remove();
		}
